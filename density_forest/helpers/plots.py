...
def imscatter(x, y, image, ax=None, zoom=1):
    im = OffsetImage(image, zoom=zoom, dpi_cor=False)
...
def plot_data(data, ax, title=None, n_clusters=None, save=False, lines_x=None, lines_y=None,
                  labels=True, minrange=1, maxrange=100, margin=0, grid_eval=None, show_data=True, means=None,
                  covs=None, imgs=None, markersize=100, textsize=25, show_ax_labels=True):
        """
        ...
        :param imgs: images to plot instead of class labels
        ...
        if show_data:
            if labels:
                ...
                for i, c in enumerate(data):
                    ...
                    ax.scatter(c[:, 0], c[:, 1], s=markersize, color=color_cluster, zorder=1)
                    x = c[:, 0]
                    y = c[:, 1]
                    n = [int(c) for c in c[:, 2]]
                    if imgs is None:
                        for j, txt in enumerate(n):
                            ax.text(x[j], y[j], txt, size=textsize)
                    else:  # overlay images
                        for j, txt in enumerate(n):
                            imscatter(x[j], y[j], imgs[txt - 1], ax, zoom=.2)
            else:
        ...
        ax.grid(color=(1, 0.4, 0), markeredgewidth=3)
        if lines_x is not None:
            _ = [ax.axvline(x=x_line, c="red") for x_line in lines_x]
        if lines_y is not None:
            _ = [ax.axhline(y=y_line, c="red") for y_line in lines_y]
        ...
        ax.set_xlim([minrange[0] - margin, maxrange[0] + margin])
        ax.set_ylim([minrange[1] - margin, maxrange[1] + margin])

        if show_ax_labels is False:
            ax.set_xticklabels([])
            ax.set_yticklabels([])
