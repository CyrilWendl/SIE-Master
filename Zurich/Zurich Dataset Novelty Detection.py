...
def export_pad(img, patch_size=64, stride=40, dpi=255, s_name=None, cmap=cm.RdYlGn):
    """
    ...
    for x in range(1, rows):
        ...
        # patch
        rect = Rectangle((0, x * stride), patch_size, patch_size, linewidth=1, facecolor=color_win, edgecolor='black', alpha=.2)
        ax.add_patch(rect)
        # central part
        rect = Rectangle((pad, pad + (x * stride)), stride, stride, linewidth=1, facecolor=color_win, edgecolor='black', lw=.5,
                         fill=True)
        ax.add_patch(rect)
    ...
    for x in range(cols):
        color_win = next(color)
        # patch
        rect = Rectangle((x * stride, 0), patch_size, patch_size, linewidth=1, facecolor=color_win, edgecolor='black', alpha=.2)
        ax.add_patch(rect)
        # central part
        rect = Rectangle((pad + (x * stride), pad), stride, stride, linewidth=1, facecolor=color_win, edgecolor='black', lw=.5,
                         fill=True)
        ax.add_patch(rect)
